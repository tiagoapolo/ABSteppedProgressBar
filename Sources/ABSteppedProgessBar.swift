...
//
//  Created by Tiago Paiva on 23/10/17.
//  Copyright (c) 2017 Tiago Paiva. All rights reserved.
//
...
@IBDesignable open class ABSteppedProgressBar: UIView {
    ...
    fileprivate func _renderTextIndexes() {
        ...
        for i in 0...(self.numberOfPoints - 1) {
            ...
            if let text = self.delegate?.progressBar?(self, textAtIndex: i) as? String {
            ...
            } else if let text = self.delegate?.progressBar?(self, textAtIndex: i) as? UIImage {
                ...
                imageView.frame = CGRect(x: centerPoint.x - imageView.bounds.width/2, y: centerPoint.y - imageView.bounds.height/2, width: imageView.bounds.width, height: imageView.bounds.height)
